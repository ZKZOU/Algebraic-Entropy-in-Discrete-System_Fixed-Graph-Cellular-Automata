...
### Clarification on the Mathematical Form: 

Although the multiplicative entropy S = ∏ᵢ₌₁ᴺ mᵢ formally resembles the product of a joint probability distribution, it operates on absolute energy integers mᵢ ∈ ℕ⁺ under the constraint ∑ᵢ₌₁ᴺ mᵢ = E₀ (conserved total energy), not on normalized probabilities ∑ᵢ pᵢ = 1. Consequently, its domain, monotonicity, and physical interpretation are fundamentally distinct. It is not a measure of statistical correlation or information content, but a dynamic scalar generated by the irreversible energy homogenization process on a topologically fixed network. 
